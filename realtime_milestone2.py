...
# Load the CNN model using TensorFlow's load_model
cnn_model = load_model('CNN_Model.h5')

# Load label encoder as a NumPy array
label_encoder = np.load('label_encoder.npy')

# Convert NumPy array to a dictionary for mapping
label_mapping = {i: label for i, label in enumerate(label_encoder)}
...
def perform_prediction(pitch_model, yaw_model, roll_model, img):
    ...
    pitch_rad, yaw_rad, roll_rad = model_predict(pitch_model, yaw_model, roll_model, features)

    # Convert from radians to degrees
    pitch = np.degrees(pitch_rad[0])
    yaw = np.degrees(yaw_rad[0])
    roll = np.degrees(roll_rad[0])

    # Draw axis using degree values
    frame = draw_axis(img, pitch_rad, yaw_rad, roll_rad, nose_x, nose_y)

    return frame, pitch, yaw, roll
# --- Function to classify pose based on CNN model ---
def classify_pose_cnn(pitch_rad, yaw_rad, roll_rad, cnn_model, label_encoder):
    angle_features = np.array([[pitch_rad, yaw_rad, roll_rad]], dtype='float32')
    predictions = cnn_model.predict(angle_features)
...
while cap.isOpened():
        ...
        # Read the next frame from the video
            ...
            # Classify the pose using the CNN model
            pose_class = classify_pose_cnn(pitch, yaw, roll, cnn_model, label_mapping)
        ...
        # Add a square border around the face
            ...
            if nose_x.size > 0 and nose_y.size > 0:
                ...
                cv2.putText(frame, f'Pose: {pose_class}', (40, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
